...
bigass_df <- left_join(NEI, SCC) %>%
  arrange(desc(Emissions)) # Ah! Sorting by emissions shows that forest fires are the largest contributors to emissions. Incorporating this finding in the first graph.
...
# Define function that takes an SCC code as a character and returns corresponding Short.Name value (character) from SCC df
# (note that SCC codes are class "character" in NEI and class "factor" in SCC.)
sccLookup <- function(scc) {
  dplyr::filter(SCC, SCC == scc)$Short.Name %>% as.character()
}

# sccToClass("10100101") # Test: it works. Neat.
```
