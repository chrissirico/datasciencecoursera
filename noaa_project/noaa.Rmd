...
stack_data_human <- injuries_and_deaths_per_event %>% 
  ...
  select(deaths, injuries, cumulative_deaths, cumulative_injuries, cumulative_casualties, count, everything()) %>%
...
stack_data_human %>% 
  filter(cumulative_casualties <= .95*sum(stack_data_human$casualty_count)) %>%
ggplot() +
  geom_col(aes(forcats::fct_reorder(EVTYPE, casualty_count, .desc = TRUE), casualty_count)) +
  # Vertical x labels
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = .5)) +
  labs(
